...
import { MAX_IMAGE_SIZE_BYTES, validateImageFile } from "@/lib/uploads/validation";
...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    const { dir, urlPrefix } = resolveImageStorageLocation();
    ...
    return new Response(JSON.stringify({ url: `${urlPrefix}/${safeName}` }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (err) {
