...
export default function AudioUploadButton({ getCursorPosition, onInsertAtPosition }: AudioUploadButtonProps): JSX.Element {
  ...
  const handleTranscribe = useCallback(() => {
    ...
    xhr.onload = () => {
      xhrRef.current = null;
      let data: { text?: string; summary?: string; error?: string } = {};
      try {
      ...
      // Gemini returns both a structured summary and the raw transcript in
      // one request — insert the summary first (immediately useful) with
      // the full transcript right below it, rather than picking one.
      const summaryHtml = data.summary && data.summary.trim().length > 0 ? textToInlineDoc(data.summary) : "";
      const transcriptHtml = textToInlineDoc(data.text);
      const html = summaryHtml
        ? `${summaryHtml}<h2>Transcripción completa</h2>${transcriptHtml}`
        : transcriptHtml;
